...
class Database(commands.Cog):
    ...
    def add_symbol(self, server: discord.Guild, member: discord.Member, symbol: str, count : int = 1):
        ...
        if symbol in user_db.keys():
            ...
            user_db[symbol] = count
        self.save()
    def reset_cooldown(self, server: discord.Guild, member: discord.Member, cooldown : int = 600):
        user_db = self.get_member(server, member)
    ...
    def on_cooldown(self, server: discord.Guild, member: discord.Member):
        ...
        if "cooldown" in user_db.keys():
            ...
            return False
    def has_symbol(self, server: discord.Guild, member: discord.Member, symbol: str, count: int = 1):
        user_db = self.get_member(server, member)
